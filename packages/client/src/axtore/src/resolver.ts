...
const getObservableQuery = (
  ...
  const observableQueries = createProp(
    query,
    "observableQueries",
    createEntitySet<ObservableQuery>
  );
  return observableQueries.get(queryOptions.variables, () =>
    client.watchQuery(queryOptions)
  );
};
  ...
  const queryContext: QueryContext = {
    ...
    get(input?: Query | Atom, options?: any): any {
      ...
      if (isQuery(input)) {
        ...
        return connectedQuery.get(options).then((result) => {
          // should subscribe only if the session is active
          if (session && isActive?.()) {
            session.onDispose.set(
              {},
...
const refetchQuery = async (
  client: Client,
  queryOptions: QueryOptions,
  { always = true, all }: { always?: boolean; all?: boolean } = {}
) => {
  if (!all) {
    client.watchQuery(queryOptions);
  }

  const obs: ObservableQuery[] = [];
  const hasVariables = Object.keys(queryOptions.variables ?? {}).length > 0;
  await client.refetchQueries({
    include: [queryOptions.query],
    onQueryUpdated(oq) {
      // do refetching if refetchAlways = true or the query is called only
      if (!always && !oq.getLastResult()) {
        return false;
      }

      if (!all) {
        // if variables does not match, skip refetching
        if (hasVariables && !equal(queryOptions.variables, oq.variables)) {
          return false;
        }
      }
  ...
  const result = obs[0]?.getCurrentResult();
...
const evictQuery = (
  ...
  Object.entries(fields).forEach(([field, variables]: [string, any]) => {
    ...
    const args = variables === true ? {} : variables || defaultVariables;
    const result = client.cache.evict({
      id: "ROOT_QUERY",
      fieldName: originalField,
      // IMPORTANT: don't pass args if it is empty object
...
const createMutationContext = (
  ...
  return {
    ...
    refetch(query, ...args) {
      ...
      return refetchQuery(client, query.use(client).mergeOptions(options));
    },
  };
};
  ...
  if (isQuery(owner)) {
    return async (value: any, args: any, context: any, _: any) => {
      ...
      const session = sessions.get(args, () =>
        createSession(
          client,
          owner.use(client).mergeOptions({ variables: args })
        )
      );
      ...
      if (isLazy(result)) {
        ...
        const fetchLazyValue = () => {
          ...
          lazy.loader().then((lazyValue) => {
            if (!isActive()) return;
            const queryOptions = owner.use(client).mergeOptions();
