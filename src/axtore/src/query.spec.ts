...
describe("static query", () => {
  test("should throw an error if specific named query not found", () => {
    expect(() =>
      baseStore.use("GetProductsXXX", ({ query }) =>
        query<GetProductsVariables, GetProductsData>()
      )
    ).toThrowError(
  ...
  test("static query", async () => {
    ...
    const store = baseStore.use("GetProducts", (x) =>
      x.query<GetProductsVariables, GetProductsData>()
    );
...
describe("dynamic query", () => {
  test("single field query", async () => {
    const client = createClient();
    const store = baseStore.use("GetProducts", ({ query }) =>
      query<GetProductsVariables, GetProductsData>({
        resolve: {
          products: (args: GetProductsVariables) =>
  ...
  test("using alias", async () => {
    const client = createClient();
    const store = baseStore.use("GetProductsWithAlias", ({ query }) =>
      query<GetProductsVariables, GetProductsWithAliasData>({
        resolve: {
          "fieldAlias:resolverName": (args: GetProductsVariables) =>
            products.slice(0, args.limit),
        },
      })
    );

    const r1 = await store.defs.GetProductsWithAlias.use(client).get({
      variables: { limit: 1 },
    });

    const r2 = await store.defs.GetProductsWithAlias.use(client).get({
      variables: { limit: 2 },
    });

    expect(r1).toEqual({ fieldAlias: products.slice(0, 1) });
    expect(r2).toEqual({ fieldAlias: products.slice(0, 2) });
  });

  test("multiple fields query", async () => {
    const client = createClient();
    const store = baseStore.use("GetProductGroup", ({ query }) =>
      query<GetProductsVariables, GetProductGroupsData>({
        resolve: {
          one: (args: GetProductsVariables) => products.slice(0, args.limit),
          two: (args: GetProductsVariables) => products.slice(args.limit),
        },
      })
    );

    const r = await store.defs.GetProductGroup.use(client).get({
      variables: { limit: 1 },
    });

    expect(r).toEqual({ one: products.slice(0, 1), two: products.slice(1) });
  });
  ...
  test("auto type mapping", async () => {
    ...
    const store = baseStore.use("GetDetailedProducts", ({ query }) =>
      query<GetProductsVariables, GetDetailedProductsData>({
        resolve: {
          products: [
            () => [
  ...
  test("change listener should be removed once the query refetches", async () => {
    ...
    const store = baseStore
      .use("Counter", ({ atom }) => atom(1))
      .use("GetValue", ({ query }, { Counter }) =>
